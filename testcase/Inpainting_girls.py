...
def main():
    ...
    s=window_stride
    orimg = cv2.imread("grilsrestored.png", 0)
    image = cv2.imread("girlsorig.png", 0)
    orimg = cv2.resize(orimg,(330,171))
    ...
    print(r)
    print(c)
    mask = cv2.imread('mask_girls_all.png',0)
    mask[mask!=255]=0
    mask[mask==255]=1
    ...
    cv2.imwrite("girlsorig.png".split(".")[0]+"inpainting.png", inpainting_image.astype('uint8'))
...
a = cv2.imread("girlsoriginpainting.png", 0)
plt.imshow(a,"gray")


# In[10]:

b = cv2.imread("girlsorig.png", 0)
plt.imshow(b,"gray")
