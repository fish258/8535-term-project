...
def main():
    ...
    s=window_stride
    orimg = cv2.imread("a4.png")
    orimg=cv2.resize(orimg,(114,135))
    orimg=orimg[:,:,2]#[:,:,0] (b-channel); [:,:,1] (r-channel)
    image = cv2.imread("a2.png")
    image=cv2.resize(image,(114,135))
    image=image[:,:,2]#
    ...
    print(r)
    print(c)
    mask=cv2.imread("a3.png",0)
    mask=cv2.resize(mask,(114,135))
    #glasses case
    mask[mask<200]=1
    mask[mask>=200]=0
    ...
    cv2.imwrite("a2.png".split(".")[0]+"inpainting_r.png", inpainting_image.astype('uint8'))# "inpainting_b", "inpainting_g"
...
a = cv2.imread("a2inpainting_r.png", 0)#"a2inpainting_b", "a2inpainting_g"
plt.imshow(a,"gray")


# In[10]:

b = cv2.imread("a2.png", 0)
plt.imshow(b,"gray")
